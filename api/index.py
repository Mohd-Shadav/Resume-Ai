...
# ✅ FIX: Using Argon2 (no password length limit)
...
async def signup(user: models.UserCreate):
    try:
        ...
        # ✅ GET SAVED USER
        saved_user = db.users[user_id]

        token = create_access_token({"user_id": user_id})

        return {
            "token": token,
            "user": models.UserOut(**saved_user)
        }
...
async def get_jobs(current_user: dict = Depends(get_current_user)):
    ...
    # ✅ If employer → show their jobs
    if current_user["role"] == "employer":
        return [
            models.JobOut(**j)
            for j in db.db.get_jobs(current_user["id"])
        ]

    # ✅ If job seeker → show matched jobs
    user_resumes = db.db.get_resumes(current_user["id"])

    if not user_resumes:
        return []

    latest_resume = user_resumes[-1]["content"].lower()
    ...
    for job in db.jobs.values():
        ...
        for skill in job["requirements"]:
            if skill.lower() in latest_resume:
                score += 1

        if score > 0:
            job["match_score"] = (score / len(job["requirements"])) * 100
